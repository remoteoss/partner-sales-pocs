...
// it — the API resolves its `sub` as a user slug — so any user-scoped endpoint
// fails with "User not found with the given slug: <client_id>" or "Company not
// found". Only genuinely pre-company calls may use it.
describe('getAuthType', () => {
  it('pre-company endpoints use the partner token', () => {
    expect(getAuthType('/v1/companies', false)).toBe('partner');
    expect(getAuthType('/v1/companies/schema', false)).toBe('partner');
  ...
  it('strips the query string before matching', () => {
    ...
    expect(getAuthType('/v1/countries?limit=200', false)).toBe('partner');
  });
  ...
  it('a partner token is never used for a user-scoped path, session or not', () => {
    for (const available of [true, false]) {
      expect(getAuthType('/v1/employments', false, available)).not.toBe('partner');
      expect(getAuthType('/v1/companies/887cf22a', false, available)).not.toBe('partner');
    }
  });
  ...
  it('sessionAvailable defaults to false', () => {
