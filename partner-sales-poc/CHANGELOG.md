...
| Issue | Impact |
|-------|--------|
| **`npm run build` fails** — `tsconfig.json` references `tsconfig.node.json`, which sets `noEmit: true` and never sets `composite: true`, so `tsc -b` aborts with TS6306 and TS6310 | Blocks deploying. Pre-existing on `main`; both tsconfigs are untouched by this branch. The build dies before typechecking a single source file, so there may be further errors hidden behind it. `npm run dev` is unaffected because Vite doesn't typecheck |
| **`npm run lint` fails** — 16 errors across `OnboardingSDK.tsx` (ref accessed during render, unused vars), `CreateCompany.tsx` and `JsonSchemaForm.tsx` | Pre-existing on `main`. Would fail CI if lint were wired into one |
